                    ...
                    <div className="absolute top-0 left-0 w-full z-20 p-4 flex justify-between items-start">
                        <div className="flex flex-col gap-2">
                            <div className="flex flex-col">
                                <span className="text-4xl font-black italic tabular-nums text-transparent bg-clip-text bg-gradient-to-b from-white to-cyan-300 drop-shadow-[2px_2px_0_#ff00ff]">
                                    {data.score.current.toString().padStart(6, '0')}
                                </span>
                                <span className="text-xs font-bold text-pink-500 tracking-[0.5em]">SCORE</span>
                            </div>
                            {data.score.high > 0 && (
                                <div className="flex flex-col -mt-1 opacity-80">
                                    <span className="text-sm font-bold italic tabular-nums text-yellow-400">
                                        {data.score.high.toString().padStart(6, '0')}
                                    </span>
                                    <span className="text-[10px] font-bold text-yellow-600 tracking-widest">HI-SCORE</span>
                                </div>
                            )}
                        </div>
