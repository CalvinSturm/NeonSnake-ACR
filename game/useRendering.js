...
import { FoodType, EnemyType, GameStatus } from '../types';
// ─────────────────────────────
// DRAWING HELPERS
// ─────────────────────────────
const drawNeonStroke = (ctx, color, width, blur = 10) => {
...
const drawNeonRect = (ctx, x, y, w, h, color, blur = 10) => {
    ctx.shadowColor = color;
    ctx.shadowBlur = blur;
    ctx.fillStyle = color;
...
// ─────────────────────────────
const drawTransition = (rc, startTime) => {
...
const drawTerminals = (rc, terminals, snakeHead) => {
    const { ctx, gridSize, halfGrid, now } = rc;
    const PI2 = Math.PI * 2;
    terminals.forEach(t => {
...
const drawEnemies = (rc, enemies, snakeHead) => {
    const { ctx, gridSize, halfGrid, now } = rc;
    const PI2 = Math.PI * 2;
...
const drawProjectiles = (rc, projectiles) => {
    const { ctx, now } = rc;
    const PI2 = Math.PI * 2;
...
const drawParticles = (rc, particles) => {
    const { ctx } = rc;
...
const drawShockwaves = (rc, shockwaves) => {
    const { ctx } = rc;
    const PI2 = Math.PI * 2;
...
const drawLightning = (rc, lightningArcs) => {
    const { ctx } = rc;
...
const drawFloatingText = (rc, texts) => {
    const { ctx } = rc;
...
const drawDigitalRain = (rc, drops) => {
    const { ctx } = rc;
    ctx.fillStyle = '#0f0';
