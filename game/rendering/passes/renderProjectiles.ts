/**
 * Render Projectiles Pass
 * 
 * Handles all projectile types with dual-mode Canvas2D/WebGL rendering.
 * WebGL used for static projectile types; Canvas2D for animated LANCE.
 */

import { RenderContext } from '../types';
import { Projectile } from '../../../types';
import { isWebGLReady } from '../../../graphics';
import { renderProjectilesWebGL } from '../../../graphics/renderers/projectiles';

/**
 * Render all projectiles.
 * Uses WebGL when available for static types, Canvas2D for animated LANCE.
 */
export const renderProjectiles = (rc: RenderContext, projectiles: Projectile[]) => {
    const { ctx, now } = rc;

    // Try WebGL rendering first
    let fallbackProjectiles = projectiles;

    if (isWebGLReady()) {
        const result = renderProjectilesWebGL(projectiles);
        fallbackProjectiles = result.fallback; // Only LANCE types need Canvas2D
    }

    // Canvas2D rendering for fallback projectiles
    for (const p of fallbackProjectiles) {
        ...
        if (p.type === 'LANCE') {
            // Animated procedural effect - must use Canvas2D
            const length = 60;
            ...
            ctx.shadowBlur = 5;
            for (let i = -1; i <= 1; i++) {
                ctx.fillRect(i * 18, -width * 1.5, 3, width * 3);
            }
