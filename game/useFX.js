...
export function useFX(game) {
    const { particlesRef, floatingTextsRef, shockwavesRef, lightningArcsRef, shakeRef, digitalRainRef, chromaticAberrationRef } = game;
    // ─────────────────────────────────────────────
    // CAMERA SHAKE (SPRING-BASED)
    // ─────────────────────────────────────────────
    const translationRef = useRef({ x: 0, y: 0 });
    const velocityRef = useRef({ x: 0, y: 0 });
    const triggerShake = useCallback((x, y) => {
        // Camera shake disabled by request
    }, []);
    const tickTranslation = useCallback((dt) => {
    ...
    // FX EMITTERS
    // ─────────────────────────────────────────────
    const spawnFloatingText = useCallback((x, y, text, color = COLORS.damageText, size = 12) => {
    ...
    const triggerShockwave = useCallback((wave) => shockwavesRef.current.push(wave), [shockwavesRef]);
    const triggerLightning = useCallback((arc) => lightningArcsRef.current.push(arc), [lightningArcsRef]);
    // ─────────────────────────────────────────────
    ...
    const updateFX = useCallback(() => {
        ...
        if (chromaticAberrationRef.current > 0) {
            ...
            if (chromaticAberrationRef.current < 0.01)
