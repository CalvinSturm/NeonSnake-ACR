...
import { FoodType, FoodItem, EnemyType, Point } from '../types';
import {
  CHANCE_BONUS, CHANCE_POISON, CHANCE_SLOW, CHANCE_MAGNET, CHANCE_COMPRESSOR,
...
export function useSpawner(
  ...
  const {
    ...
    enemySpawnTimerRef, terminalSpawnTimerRef, audioEventsRef
  } = game;

  // ─────────────────────────────
  // FOOD
  // ─────────────────────────────
  const spawnFood = useCallback((forceType?: FoodType) => {
    ...
    if (!forceType) {
      ...
      let c = CHANCE_BONUS;
      if (roll < c) type = FoodType.BONUS;
      else if (roll < (c += CHANCE_POISON)) type = FoodType.POISON;
      else if (roll < (c += CHANCE_SLOW)) type = FoodType.SLOW;
      else if (roll < (c += CHANCE_MAGNET)) type = FoodType.MAGNET;
      else if (roll < (c += CHANCE_COMPRESSOR)) type = FoodType.COMPRESSOR;
    } else {
      type = forceType;
    }
  ...
  const spawnTerminal = useCallback(() => {
    if (terminalsRef.current.length > 0) return;
  ...
  // ENEMY
  // ─────────────────────────────
  const spawnEnemy = useCallback((forcedType?: EnemyType) => {
    ...
    do {
      ...
      Math.abs(pos.x - head.x) + Math.abs(pos.y - head.y) < 10 &&
      attempts < 15
    );
