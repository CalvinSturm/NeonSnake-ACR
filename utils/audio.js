...
class HackVoice {
    ...
    update(progress) {
        if (!this.isPlaying || !this.filter || !this.lfo || !this.lfoGain || !this.gain || !this.osc)
            return;
        const t = this.ctx.currentTime;
        // Progress 0.0 -> 1.0
...
class MusicEngine {
    ...
    createNoiseBuffers() {
        ...
        for (let i = 0; i < snareSize; i++)
            snareData[i] = (Math.random() * 2 - 1) * Math.exp(-i / (sr * 0.03));
        ...
        for (let i = 0; i < hatSize; i++)
            hatData[i] = (Math.random() * 2 - 1) * Math.exp(-i / (sr * 0.005));
    }
    setVolume(vol) {
    ...
    setHackProgress(progress) {
        ...
        if (Math.abs(this.hackProgress - p) < 0.01 && p !== 0 && p !== 1)
            return;
        if (p > 0) {
    ...
    // 🥁 DRUMS
    // ─────────────────────────────
    scheduleDrums(beat, time, threat, stage) {
        ...
        if (threat === THREAT_LEVELS.LOW) {
            if (beat % 4 === 2)
                this.playHat(time, 0.1);
        }
        else if (threat === THREAT_LEVELS.MEDIUM) {
            if (beat % 2 === 0)
                this.playHat(time, 0.15);
        }
        else {
    ...
    // 🎸 BASS
    // ─────────────────────────────
    scheduleBass(beat, time, threat, stage) {
        const isDownbeat = beat % 4 === 0;
        const isOffbeat = beat % 4 === 2;
        const root = ROOT_FREQ / 4;
        if (stage === STAGES.INTRO) {
            if (beat === 0)
                this.playBassNote(time, root, 2.0 * this.beatTime, 300);
        }
        else if (threat === THREAT_LEVELS.LOW) {
            if (isDownbeat)
                this.playBassNote(time, root, 0.5 * this.beatTime, 300);
        }
            ...
            if (isOffbeat)
                this.playBassNote(time, root, 0.4 * this.beatTime, 500);
        }
    }
    // ─────────────────────────────
    // 🎹 MELODY / ARP
    // ─────────────────────────────
    scheduleMelody(beat, time, threat, stage) {
    ...
    playBassNote(time, freq, dur, cutoff) {
        const osc = this.ctx.createOscillator();
        osc.type = 'sawtooth';
        const filter = this.ctx.createBiquadFilter();
        filter.type = 'lowpass';
    ...
    onBar(cb) { this.barCallbacks.push(cb); }
    clearCallbacks() { this.beatCallbacks = []; this.barCallbacks = []; }
    play(event, data) {
        if (!this.ctx || this.ctx.state === 'suspended')
            this.resume();
        switch (event) {
            case 'MOVE':
                this.playTone(150, 'triangle', 0.05, 0, true, 0.05);
                break;
            case 'EAT':
                this.playEat(data?.multiplier ?? 1);
                break;
            case 'XP_COLLECT':
                this.playTone(1200, 'sine', 0.05, 0, true, 0.1);
                break;
            case 'SHOOT':
                this.playTone(800, 'square', 0.05, 0, true, 0.1);
                break;
            case 'EMP':
                this.playTone(50, 'sawtooth', 1.0, 0, true, 0.5);
                break;
            case 'HIT':
                this.playTone(200, 'sawtooth', 0.1, 0, true, 0.2);
                break;
            case 'GAME_OVER':
                this.playTone(80, 'sawtooth', 1.5, 0, true, 0.6);
                break;
            case 'LEVEL_UP':
                this.playLevelUp(data?.level || 1, data?.difficulty || 'EASY', data?.combo || 1);
                break;
            case 'BONUS':
                this.playTone(880, 'square', 0.3, 0, false, 0.2);
                break;
            case 'POWER_UP':
                this.playTone(400, 'sine', 0.5, 0, true, 0.3);
                break;
            case 'SHIELD_HIT':
                this.playTone(1200, 'square', 0.1, 0, true, 0.2);
                break;
            case 'ENEMY_DESTROY':
                this.playEnemyDeath();
                break;
            case 'HACK_LOST':
                ...
                break;
            case 'HACK_COMPLETE':
                this.hackVoice?.implode();
                break;
            case 'ENEMY_SPAWN':
                this.playTone(80, 'sawtooth', 0.5, 0, true, 0.2);
                break;
            case 'COMPRESS':
                this.playTone(600, 'sawtooth', 0.1, 0, true, 0.2);
                break;
        }
    }
    playLevelUp(level, difficulty, combo) {
    ...
    playTone(freq, type, duration, startTime = 0, ramp = true, vol = 1.0) {
        if (!this.ctx || !this.sfxGain)
            return;
        const osc = this.ctx.createOscillator();
        const gain = this.ctx.createGain();
