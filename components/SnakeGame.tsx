...
import { GameStatus, Difficulty, CharacterProfile, Direction, EnemyType } from '../types';
...
const SnakeGame: React.FC = () => {
  ...
  /* ─────────────────────────────
     ...
     ───────────────────────────── */
  const currentTheme =
    STAGE_THEMES[((uiStage - 1) % 4) + 1] || STAGE_THEMES[1];
...
const handleMobileEMP = useCallback(() => {
  ...
  /* ─────────────────────────────
     ...
     ───────────────────────────── */
  return (
    <div className="relative w-full h-full bg-[#050505] flex flex-col items-center justify-center p-2 overflow-hidden selection:bg-cyan-500/30">
      
      {/* ────────────────── HEADER HUD ────────────────── */}
      <div className="w-full max-w-[800px] grid grid-cols-12 gap-4 mb-4 items-end relative z-10">
        
        {/* LEFT: Score & Combo */}
        <div className="col-span-3 flex flex-col items-start">
          <div className="text-[10px] text-cyan-700 tracking-[0.2em] font-bold mb-1">RUNTIME_METRICS</div>
          <div className="relative group">
            {/* Score Display (Uncontrolled, updated via ref) */}
            <div 
              ref={scoreDisplayRef} 
        ...
        <div className="col-span-6 flex flex-col items-center justify-end pb-1">
           ...
           <div className="flex gap-2 mt-2 h-5">
              {isShockReady && (
                 <div className="flex items-center gap-1 px-1.5 py-0.5 bg-cyan-950/40 border border-cyan-500/30 rounded text-[9px] text-cyan-300 animate-pulse">
                    <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full shadow-[0_0_5px_cyan]"></div>
                    EMP_READY
                 </div>
              )}
              {uiShield && (
      ...
      {/* ────────────────── MAIN CANVAS FRAME ────────────────── */}
      <div className="relative group p-1 bg-gradient-to-b from-gray-800 to-gray-900 rounded-md shadow-[0_0_40px_rgba(0,0,0,0.8)] border border-gray-700" style={{ borderColor: currentTheme.wall }}>
         {/* Decoration Bolts */}
             ...
             <div className="absolute inset-0 bg-black/95 flex flex-col items-center justify-center z-40 backdrop-blur-lg p-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl">
                    {upgradeOptions.map((opt, idx) => (
                        <button key={opt.id} onClick={() => progression.applyUpgrade(opt.id as UpgradeId)} className="flex flex-col p-8 bg-gray-900/80 border-2 border-gray-700 hover:border-white hover:bg-gray-800 transition-all text-left group relative">
                            <div className="absolute -top-3 -left-3 w-8 h-8 bg-black border-2 border-gray-700 flex items-center justify-center text-xs font-bold text-gray-500 group-hover:text-white group-hover:border-white">{idx+1}</div>
                            <div className="text-4xl mb-4 self-center filter drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">{opt.icon}</div>
                            <h3 className={`text-2xl font-bold font-display mb-2 ${opt.color}`}>{opt.title}</h3>
                            <p className="text-base text-gray-300 font-mono leading-relaxed">{opt.description}</p>
                        </button>
                    ))}
                </div>
             </div>
      ...
      <div className="w-full max-w-[400px] mt-6 grid grid-cols-3 gap-3 md:hidden h-40 select-none">
          ...
          <div className="col-start-2 row-start-2 bg-gradient-to-b from-red-900/40 to-red-950/60 rounded-lg border-b-4 border-red-950 active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center shadow-lg active:bg-red-900/50 border border-red-900/50" onPointerDown={(e) => { e.preventDefault(); handleMobileEMP(); }}> 
            <span className="font-bold text-red-400 tracking-tighter text-sm">EMP</span> 
          </div>
      ...
      <div className="mt-6 hidden md:flex w-full max-w-[800px] justify-between items-center text-[10px] text-gray-500 font-mono border-t border-gray-800/50 pt-4">
          ...
          <div className="flex gap-6">
             ...
             <div className="flex flex-col gap-1">
                <span className="text-cyan-700 font-bold tracking-wider mb-1">COMBAT</span>
                <div className="flex gap-3">
                   <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded border transition-colors ${isShockReady ? "bg-cyan-900/50 border-cyan-500 text-cyan-300" : "bg-gray-800 border-gray-700 text-gray-500"}`}>SHIFT</span>
                      <span>SYSTEM SHOCK</span>
                   </div>
                   <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-gray-800 rounded border border-gray-700 text-gray-300">CLICK</span>
                      <span>PING</span>
                   </div>
                </div>
             </div>
          ...
          <div className="flex flex-col gap-1 items-end">
             ...
             <div className="flex gap-3">
                <div className="flex items-center gap-2">
                   <span className={`px-2 py-0.5 rounded border transition-colors ${isChronoReady ? "bg-indigo-900/50 border-indigo-500 text-indigo-300" : "bg-gray-800 border-gray-700 text-gray-500"}`}>Q</span>
                   <span>CHRONO</span>
                </div>
                <div className="flex items-center gap-2">
